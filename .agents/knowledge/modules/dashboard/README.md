...
The module has loading skeleton, error handling via `SessionStateScreen` with retry/clear and Telegram bot redirect, empty tables, and populated states. All components across overview, statistics, transactions, tasks, reminders, and calendar views are styled with 100% Tailwind CSS utility classes and responsive modifiers, supporting complete light and dark theme styling via `dark:`. External links across all screens (such as Google Calendar event links and external resources) open securely in new tabs (`target="_blank" rel="noopener noreferrer"`). Home surfaces attention items, quick links, and responsive grid tables; statistics and transactions screens surface financial totals, dense operational tables, interactive period filtering (`PeriodFilterToolbar` with day/week/month/quarter/year/all ranges), and visual trend summaries (`TrendSummaryStrip` with income/expense KPI metrics and `MicroBarChart` distribution bars).

### Visual Analytics Dashboard (`/analytics`)

The Analytics screen (`AnalyticsScreen`) provides a dedicated visual reporting dashboard organized into 5 specialized sub-tabs with URL query persistence (`?tab=charts|cashflow|spending|debts|records`):

- **Sub-Tab Navigation**:
  - `charts` (All Charts / Overview - default): 5 KPI summary cards (Total Income, Total Expense, Net Savings, Savings Rate, Net Debt) + 3-chart grid (Cashflow Trend 7-col, Spending Donut 5-col, Debt Structure 5-col).
  - `cashflow` (Cashflow Trend Deep-dive): Full-width 12-col Cashflow chart (height=260px) with Income/Expense/Net Balance curves + 5 cashflow summary KPI cards (Income, Expense, Net Surplus, Avg Daily Income, Avg Daily Expense) + Detailed period breakdown table (`DataTable` with period label, income, expense, surplus/deficit).
  - `spending` (Spending Breakdown): Large 240px Category Donut Chart + Category distribution table with amount, count, and proportion percentages.
  - `debts` (Debt Structure): Debt Structure comparison chart (Receivables vs Payables) + Active open debts data table with instant search and inline repay actions.
  - `records` (Detailed Records): Full operational data tables for Transactions and Debts with search, inline edit, delete, and repayment actions.
- **Interactive Multi-grain Filter**: `PeriodFilterToolbar` and `usePeriodFilter` supporting day, week, month, quarter, year, and all-time ranges with URL sync (`?period=...&ref=...`).
- **Real-time Analytics Query**: `useFinanceAnalyticsQuery({ startAt, endAt, grain })` communicating with `GET /api/finance/analytics` to aggregate historical trend buckets, category spending breakdowns, and debt ratios directly from the database.
- **Native SVG Chart Suite**:
  - `CashflowTrendChart`: Bi-color grouped bars (Income in sky, Expense in amber) paired with an overlaid Net Balance trend line, zero reference baseline, grid lines, and interactive hover tooltip.
  - `CategoryDonutChart`: SVG arc donut chart displaying top 5 spending categories by proportion plus remaining aggregated categories, paired with interactive progress bars and percentages.
