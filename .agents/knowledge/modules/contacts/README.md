...
- **Combine Contacts**: Multi-select checkboxes trigger a "Combine" action toolbar when 2+ contacts are checked. Opens a modal dialog (`apps/web/src/modules/contacts/presentation/components/combine-contacts-dialog.tsx`, re-exported via `view/combine-contacts-dialog.tsx`) allowing the user to select the primary target contact, preview/edit merged names, aliases, and addresses, and migrate all linked debts in a single backend transaction.
