...
- **Web UI & Master-Detail Architecture**: `DebtsTable` utilizes the DataTable Master-Detail pattern with single-level indented sub-panels (`renderExpandedRow`). Main table rows strictly contain top-level records to preserve continuous STT ordinal numbering (`1, 2, 3...`) and accurate multi-selection for batch consolidation. Each row containing sub-details (child debts or payment installments) renders exactly **ONE** smooth rotating Chevron dropdown button (`▶ / ▼` with `rotate-90`) in the counterparty column.
