...
- **Web UI & Hierarchical Tree**: `DebtsTable` supports multi-select checkboxes for batch combining, an expandable nested tree view with smooth rotating Chevron dropdown buttons (`▶ / ▼` with `rotate-90`), indented child rows (`↳` with `pl-4` for child debts, `pl-8` for payments under child debts), subtle child row background differentiation (`bg-slate-50/70 dark:bg-slate-900/40` for child debts, `bg-emerald-50/30 dark:bg-emerald-950/20` for payment sub-rows), parent badges (`[Gộp: N khoản con]`), child badges (`[Khoản con]`), and payment badges (`[N đợt trả]`).
- **Debt Payment History & Inline Inspection**: Every debt record (standalone, child, or settled) with payment installments (`payments.length > 0`) displays a rotating green Chevron dropdown button and summary badge (`[N đợt trả]`). The Remaining Amount column presents an intelligent progress subtitle (`Paid X ₫ (N payments)`). Expanding reveals nested payment sub-rows displaying payment amount (`- X ₫`), payment date, note, linked transaction badge (`🔗 Linked Transaction`), and a single-click delete action (`DELETE /api/debts/:id/payments/:paymentId`) that safely restores remaining debt balances. Total summary calculation preserves parent-only aggregation to eliminate double-counting.
