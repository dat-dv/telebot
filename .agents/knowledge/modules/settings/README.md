...
The preferences tab includes an opt-in React Scan control. `ReactScanProvider` persists `telebot-react-scan-enabled` in browser local storage, defaults to disabled, and dynamically imports the profiler only after the user enables it. It is a per-browser performance tool and never sends profiling data to the API.
