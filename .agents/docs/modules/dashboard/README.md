...
- Ẩn/hiện số tiền nhạy cảm trong phiên: Tiêu đề không gian làm việc (`WorkspaceHeader`) tích hợp nút chuyển đổi `Ẩn số tiền` / `Hiện số tiền` (`MoneyVisibilityProvider`). Trạng thái chỉ duy trì trong bộ nhớ React của phiên (`useState`), mặc định hiển thị khi tải trang và không lưu `localStorage`/cookie. Khi bật chế độ ẩn, các số tiền ở thẻ KPI, cột bảng, footer tổng kết và tooltip biểu đồ được che bằng mặt nạ `'••••••'`; riêng các ô input khi đang sửa trực tiếp (inline edit) vẫn giữ số thực để người dùng thao tác bình thường.
