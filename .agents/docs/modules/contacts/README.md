...
- **API & Endpoints**:
  ...
  - `combineContacts`: gọi `POST /api/contacts/combine` (`API_ROUTES.contactsCombine`) với payload `{ targetContactId, sourceContactIds, displayName, alias, descriptor, consolidateDebts }` gộp các liên hệ nguồn vào liên hệ chính, chuyển đổi và tùy chọn tự động gom nợ thành khoản nợ cha duy nhất.
- **Cache & Mutations**:
...
- **Giao diện & Thao tác (UI/UX)**:
  - Giao diện và dialog modal (`ContactsScreen` tại `presentation/components/contacts-screen.tsx` và `CombineContactsDialog` tại `presentation/components/combine-contacts-dialog.tsx`, re-exported qua `view/`) được chuẩn hóa 100% bằng Tailwind CSS utility classes, hỗ trợ chế độ dark mode (`dark:`) toàn diện. Toàn bộ liên kết ngoài (số điện thoại, địa chỉ Google Maps) mở trong tab mới với thuộc tính an toàn `target="_blank" rel="noopener noreferrer"`. Tiêu đề trang (tên trang, mô tả, nút Làm mới, nút Đăng xuất) do `WorkspaceHeader` trong common private layout cung cấp — `ContactsScreen` **không tự render header riêng**.
  ...
  - **Gộp liên hệ (Combine Contacts)**: Tích chọn từ 2 liên hệ trở lên để mở cửa sổ dialog gộp liên hệ, chọn liên hệ đích, chỉnh sửa tên/địa chỉ gộp, chọn checkbox gom nợ (`consolidateDebts` — hệ thống tự động gom nợ an toàn theo từng chiều và từng loại tiền tệ) và xác nhận an toàn.
