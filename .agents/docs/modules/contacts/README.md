...
- **Giao diện & Thao tác (UI/UX)**:
  - Giao diện và dialog modal được chuẩn hóa 100% bằng Tailwind CSS utility classes, hỗ trợ chế độ dark mode (`dark:`) toàn diện. Toàn bộ liên kết ngoài (số điện thoại, địa chỉ Google Maps) mở trong tab mới với thuộc tính an toàn `target="_blank" rel="noopener noreferrer"`.
