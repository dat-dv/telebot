...
- UI: Màn hình `SettingsScreen` được xây dựng 100% bằng Tailwind CSS utility classes (hỗ trợ dark mode `dark:`). Tiêu đề trang (tên trang, mô tả, nút Làm mới, nút Đăng xuất) do `WorkspaceHeader` trong common private layout cung cấp — `SettingsScreen` **không tự render header riêng**. Màn hình gồm 2 tab chính:
