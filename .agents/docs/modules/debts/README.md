...
- **Gộp công nợ & Cây phân cấp Cha–Con (`parentDebtId`)**: Hệ thống hỗ trợ gộp $N \ge 2$ khoản nợ có cùng chiều (`receivable` hoặc `payable`) thành 1 khoản nợ cha tổng:
  ...
  - Trên Web UI: `DebtsTable` hỗ trợ chọn nhiều dòng qua checkbox, nút bấm thao tác gộp nợ, mở rộng/thu gọn cây nợ con với nút icon Chevron xoay tinh tế (`▶ / ▼` xoay `rotate-90`), thụt lề hàng con (`↳` với `pl-4` cho khoản nợ con, `pl-8` cho đợt trả nợ của khoản con), nền phân biệt nhẹ (`bg-slate-50/70 dark:bg-slate-900/40` cho khoản con, `bg-emerald-50/30 dark:bg-emerald-950/20` cho đợt trả nợ), kèm huy hiệu `[Gộp: N khoản con]`, `[Khoản con]`, và `[N đợt trả]`. Modal `CombineDebtsDialog` hiển thị cảnh báo đỏ và khóa nút xác nhận khi phát hiện chọn lệch chiều nợ hoặc lệch loại tiền. Tổng số tiền thống kê chỉ tính các khoản cha để tránh double-counting.
- **Lịch sử đợt trả nợ & Mở rộng Inline (`payments`)**: Mọi khoản nợ (khoản đơn, khoản con, hoặc khoản đã tất toán) có phát sinh thanh toán (`payments.length > 0`) hiển thị nút mũi tên dropdown màu xanh lá và badge số đợt trả `[N đợt trả]`. Cột Số tiền còn lại hiển thị nhãn phụ tiến độ thanh toán `(Đã trả X ₫ · N đợt)`. Khi bấm mở rộng, các dòng con đợt trả nợ hiển thị trực tiếp với số tiền đợt trả (`- X ₫`), ngày thanh toán thực tế, ghi chú, icon liên kết giao dịch thu chi (`🔗 Giao dịch liên kết`), và nút xóa đợt trả nợ nhanh (`DELETE /api/debts/:id/payments/:paymentId`) giúp tự động khôi phục số nợ còn lại.
