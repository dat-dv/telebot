...
- **Gộp công nợ & Cây phân cấp Cha–Con (`parentDebtId`)**: Hệ thống hỗ trợ gộp $N \ge 2$ khoản nợ có cùng chiều (`receivable` hoặc `payable`) thành 1 khoản nợ cha tổng:
  ...
  - Trên Web UI: `DebtsTable` hỗ trợ chọn nhiều dòng qua checkbox, nút bấm thao tác gộp nợ, mở rộng/thu gọn cây nợ con (`▶ / ▼`) kèm huy hiệu `[Gộp: N khoản con]` và `[Khoản con]`. Modal `CombineDebtsDialog` hiển thị cảnh báo đỏ và khóa nút xác nhận khi phát hiện chọn lệch chiều nợ hoặc lệch loại tiền.
