...
- Hỗ trợ ghi giao dịch đơn lẻ (`create_finance_transaction`) và ghi hàng loạt nhiều khoản cùng lúc (`create_finance_transactions`). Khi câu nói có chứa tên quán ăn/cửa hàng/địa điểm (`placeName`), hệ thống tự động tìm kiếm hoặc tạo mới Contact tương ứng trong `debt_contacts` và liên kết `contactId` vào giao dịch.
...
- Giao dịch thu chi (`transactions`): Bổ sung `paymentMethod`, `currency`, `receiptUrl`, `contactId`, `placeName`.
