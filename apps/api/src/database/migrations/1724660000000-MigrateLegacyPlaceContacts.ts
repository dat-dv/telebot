...
export class MigrateLegacyPlaceContacts1724660000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    // 1. Sao chép địa điểm từ debt_contacts sang finance_places nếu chưa tồn tại
    ...
    // 2. Cập nhật place_id cho các giao dịch đang trỏ contact_id tới contact địa điểm, gỡ contact_id
    await queryRunner.query(`
    ...
    // 3. Xử lý an toàn debts nếu có liên kết nhầm với contact địa điểm
    await queryRunner.query(`
      UPDATE "debts" d
      SET "contact_id" = NULL
      FROM "debt_contacts" dc
    ...
    // 4. Xóa sạch các contact địa điểm khỏi debt_contacts
    await queryRunner.query(`
